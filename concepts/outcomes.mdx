...
To mark a call as a lead conversion, you can use either POST or GET requests with the [Conversion Pixel API](/api-reference/conversionpixel/post-v1webhooksconversion-pixel):
...
```bash
curl --request GET \
--url 'https://api.comm.com/api/v1/webhooks/conversion-pixel?phone_number=<string>&team_id=<string>&object_type=voice_call&event_type=lead' \
--header 'Authorization: Bearer <token>'
```
</Tab>
</Tabs>
...
```bash
curl --request GET \
--url 'https://api.comm.com/api/v1/webhooks/conversion-pixel?phone_number=<string>&team_id=<string>&object_type=voice_call&event_type=sale&event_profit=99' \
--header 'Authorization: Bearer <token>'
```
</Tab>
</Tabs>
